  ...
  connect() {
    if (!this.client) {
      ...
      this.client.on('error', (err) => {
        console.error(`[${USER_NAME}] HTTP/2 连接错误:`, err);
      });
    }
    return this.client;
  }
  ...
  async call(service, method, requestType, responseType, data = {}) {
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        req.destroy();
        reject(new Error(`[${USER_NAME}] 请求超时`));
      }, CONFIG.timeout);
      ...
      req.on('response', (headers) => {
        ...
        if (grpcStatus && grpcStatus !== '0') {
          ...
          reject(new Error(`[${USER_NAME}] gRPC 错误 ${grpcStatus}: ${grpcMessage || '未知错误'}`));
          return;
        }
      });
...
async function main() {
  // 校验Token是否存在
  if (!TOKEN) {
    console.error(`[${USER_NAME}] 错误：未配置Token环境变量！`);
    process.exit(1); // 退出执行
  }
  ...
  try {
    ...
    if (adventureList.data && adventureList.data.length > 0) {
      console.log(`[${USER_NAME}] 找到 ${adventureList.data.length} 个冒险任务:`);

      for (const adventure of adventureList.data) {
        console.log(`\n[${USER_NAME}] 冒险 #${adventure.id} - ${adventure.title}`);
        console.log(`[${USER_NAME}]     状态: ${adventure.status}`);
        console.log(`[${USER_NAME}]     时长: ${adventure.duration}秒`);
        ...
        console.log(`[${USER_NAME}]     奖励: ${rewards.length > 0 ? rewards.join(', ') : '无'}`);

        if (adventure.status === 'IDLE') {
          console.log(`[${USER_NAME}]     >>> 接受冒险...`);
          try {
            ...
            console.log(`[${USER_NAME}]     ✓ 冒险已接受`);
          } catch (error) {
            console.log(`[${USER_NAME}]     ✗ 接受失败: ${error.message}`);
          }
        } else if (adventure.status === 'COMPLETED') {
          console.log(`[${USER_NAME}]     >>> 领取奖励...`);
          try {
            await grpcClient.call(
              'api.adventure.Adventure',
              'FinishAdventure',
            ...
            if (!updatedAdventure) {
              console.log(`[${USER_NAME}]     >>> 冒险已完成，检查新冒险...`);
              // 检查是否有新的IDLE冒险
              const newIdleAdventure = updatedList.data?.find(a => a.status === 'IDLE');
              if (newIdleAdventure) {
                console.log(`[${USER_NAME}]     >>> 发现新冒险 #${newIdleAdventure.id}，正在接受...`);
                try {
                  await grpcClient.call(
                    'api.adventure.Adventure',
                    'AcceptAdventure',
      ...
      console.log(`[${USER_NAME}] 当前没有可用的冒险任务`);
    }
    ...
    if (makingV2Result.equipment && makingV2Result.equipment.length > 0) {
      console.log(`[${USER_NAME}] 找到 ${makingV2Result.equipment.length} 个正在制作的装备:`);
      const now = Date.now();

      for (const eq of makingV2Result.equipment) {
        console.log(`\n[${USER_NAME}] #${eq.id} - ${eq.name}`);
        if (eq.validTime) {
          const validTime = Number(eq.validTime);
          const validDate = new Date(validTime);
          console.log(`[${USER_NAME}]     完成时间: ${validDate.toLocaleString()}`);

          if (validTime <= now) {
            console.log(`[${USER_NAME}]     >>> 装备已完成，正在领取...`);
            try {
              await grpcClient.call(
                'api.props.Props',
              ...
              console.log(`[${USER_NAME}]     ✓ 装备已领取！`);
            } catch (error) {
              console.log(`[${USER_NAME}]     ✗ 领取失败: ${error.message}`);
            }
          } else {
            const remaining = Math.ceil((validTime - now) / 1000);
            console.log(`[${USER_NAME}]     正在制作中，剩余 ${remaining} 秒`);
            shouldMakeNew = false;
          }
        }
      }
    } else {
      console.log(`[${USER_NAME}] 当前没有正在制作的装备`);
    }
          ...
          if (bp.props && bp.props.length > 0) {
            ...
            if (hasEnoughMaterials(myPropsList, bp.props)) {
              ...
              try {
                ...
                console.log(`[${USER_NAME}]     ✓ 制作成功！`);
                break; // 制作一个后退出
              } catch (error) {
                console.log(`[${USER_NAME}]     ✗ 制作失败: ${error.message}`);
              }
            }
          }
        }
    ...
    console.log(`[${USER_NAME}] 等级: ${belongingsResult.level}`);
    console.log(`[${USER_NAME}] 经验: ${belongingsResult.exp} / ${belongingsResult.expMax}`);
    ...
    if (currency.length > 0) console.log(`[${USER_NAME}] 货币: ${currency.join(' | ')}`);
    ...
    if (baseStats.length > 0) console.log(`[${USER_NAME}] 基础属性: ${baseStats.join(' | ')}`);
    ...
    if (skills.length > 0) console.log(`[${USER_NAME}] 技能等级:`);
    skills.forEach(skill => console.log(`[${USER_NAME}]     ${skill}`));
    ...
    if (efficiency.length > 0) console.log(`[${USER_NAME}] 技巧: ${efficiency.join(' | ')}`);
    ...
    if (special.length > 0) console.log(`[${USER_NAME}] 特殊: ${special.join(' | ')}`);
    ...
    if (other.length > 0) console.log(`[${USER_NAME}] 其他: ${other.join(' | ')}`);
    ...
    console.error(`[${USER_NAME}] 发生错误:`, error.message);
    console.error(error.stack);
  } finally {
    grpcClient.close();
    console.log(`\n====================================`);
    console.log(`========== 用户 ${USER_NAME} 处理完成 ==========`);
    console.log(`====================================\n`);
  }
}
